...
def writeCoordinates():
    ...
    address = r"/home/sam/dev_ws/Expo4-main/CoordinatesDestination.txt"
    ...
    if pygame.mouse.get_pressed()[0] and pygame.time.get_ticks() - start_time >= debounce_delay and moveableRoomGAMEONLY.collidepoint(pygame.mouse.get_pos()):
        ...
        normx = (MOUSECOORD_X - (SCREEN_WIDTH/8))/ROOM_WIDTH
        calcx = normx*(xmax-xmin)
        destination_x = calcx + xmin

        normy = (MOUSECOORD_Y - (SCREEN_HEIGHT/8))/-ROOM_HEIGHT
        calcy = normy*(ymax-ymin)
        destination_y = calcy + ymax
        print("%s \n%s" %(destination_x , destination_y))
...
# Set the initial Goal to 0,0
with open("/home/sam/dev_ws/Expo4-main/CoordinatesDestination.txt", 'w') as file:
    file.write("%s \n%s" %(0 , 0))
